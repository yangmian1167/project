...
--		local wy = get(api..'?action=getPhone&sid='..itemid..'&token='..token..'&vno=0') 
		local wy = get(api..'?action=getPhone&sid='..itemid..'&token='..token) 
		if wy ~= nil and wy ~= '' then
			log(wy)
			phone =  string.split(wy,'|')[2]
			if #phone == 11 then
				local phone_key = (string.sub(phone,1,3)) 
				local blackPhone = {'147','182'} 
				for k,v in ipairs(blackPhone) do 
					if phone_key == v then 
					local lx_url = 'http://api.smskkk.com/api/do.php?action=addBlacklist&sid='..itemid..'&phone='..phone..'&token='..token 
					get(lx_url); 
					log("拉黑->"..phone) 
					return false 
					else
	...
	while true do
		...
		if wy ~= nil and wy ~= "" then
			if string.match(wy,"%d%d%d%d") then
				...
				sys.toast(string.match(wy,"%d%d%d%d"))
				return string.match(wy,"%d%d%d%d")
...
function laixin_cancelRecv(phone) --释放指定手机号
	local wy = get(api..'?action=cancelRecv&phone='..phone..'&sid='..itemid..'&token='..token)
	log(wy)
	if wy and wy == 'success' then
		sys.toast('释放成功')
	end
...
page.注册界面={{{369, 147, 0xffffff},{258, 140, 0xff615c},{219,  69, 0xff6c54},{485, 188, 0xff5d76},}, 85, 92, 53, 558, 204}
page.登录界面={{{387, 147, 0xff686d},{252, 128, 0xffffff},{130, 121, 0xffffff},{318, 785, 0xdf0f4b},{327, 834, 0xdf0f4b},}, 85, 102, 55, 554, 863}
